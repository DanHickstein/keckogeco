...
def test_sim_temperatures_and_positions():
    cfg = DeviceConfig(key="daq", driver="usb2408", address="205F843")
    ...
    with daq:
        ...
        assert daq.positions == DEFAULT_POSITIONS["205F843"]
...
def test_unknown_serial_gets_placeholder_positions():
    cfg = DeviceConfig(key="daq", driver="usb2408", address="0BADF00")
    daq = USB2408.from_config(cfg, sim=True)
    assert daq.positions == ["?"] * NUM_CHANNELS
