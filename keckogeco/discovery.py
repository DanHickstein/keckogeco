"""Instrument auto-discovery for the KeckLFC rack (``python -m keckogeco.discovery``).
...
Usage::
    ...
    python -m keckogeco.discovery                # verify config, then discover the rest
    python -m keckogeco.discovery --rescan       # ignore config, full discovery
    python -m keckogeco.discovery --no-gpib      # skip the VISA/GPIB scan
    python -m keckogeco.discovery --ports COM3 COM17
    python -m keckogeco.discovery -v             # show all probe responses
"""
...
def save_config(instruments: dict, config_path: Path) -> None:
    ...
    if config_path.exists():
        ...
        doc.add(
            tomlkit.comment("Generated by python -m keckogeco.discovery. See config/README.md.")
        )
